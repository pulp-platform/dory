...
void __attribute__((noinline)) xpulp_nn_conv_u8_u4_i8_4x4(
                        uint8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  uint8_t * pIm2ColBase = pIm2ColBuffer + (4 * core_id * PACK_INT8_SIZE(ch_in) * dim_kernel_x * dim_kernel_y);
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    for(i_out_x=(section * dim_out_x_r); i_out_x<(dim_out_x_r + (section * (dim_out_x_r + flag_dim_out_x_odd))); i_out_x++)
    {
      ...
      {
        pOutBuffer = xpulp_nn_matmul_u8_u4_i8_4x4(
          pIm2ColBase,
          pBias,
          pOutBuffer,
          pOutBuffer + ch_out_r,
