...
void __attribute__ ((noinline))  xpulp_nn_avgpool_u4_i4(
  uint8_t * pIn,
  int8_t * pOut,
  int32_t lambda,
  uint16_t out_shift,
              ...
              int8_t out_el = 0;
                for (k_y = k_y_start; k_y < k_y_end; k_y++)
                {
                    for (k_x = k_x_start; k_x < k_x_end; k_x++)
                    {
                        ...
                        sum[0] += (uint32_t) bitext_u((unsigned int) cur_chans, 4, 0);

                        sum[1] += (uint32_t) bitext_u((unsigned int) cur_chans, 4, 4);
                    }
                }
                int32_t out_large;
                if (flag_requant) {
