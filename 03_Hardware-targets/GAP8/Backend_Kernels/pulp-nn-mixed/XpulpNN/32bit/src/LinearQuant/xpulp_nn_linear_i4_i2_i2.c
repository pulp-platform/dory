...
void __attribute__((noinline)) xpulp_nn_linear_i4_i2_i2(
                        int8_t *pIn,
                        int8_t *pBias,
                        int8_t *pOut,
                        int8_t *pWeight,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(i=start; i<stop; i+=4)
  {
    ...
    for(int j=0; j < (dim_vec >> 4); j++)
    {
      ...
      sum3 = MacLoads8(1, 0, 2, 0, ptrA3, sum3);
      ptrA3 = MacLoadUpdate(ptrA3);
      sum4 = MacLoads8(1, 0, 3, 0, ptrA4, sum4);
    ...
    if(col_cnt)
    {
      ...
      do
      {
        ...
        pB++;
        int8_t inB3 = (int8_t) bitext((int32_t) *pB, 4, 0);
        int8_t inB4 = (int8_t) bitext((int32_t) *pB, 4, 4);
        pB++;
