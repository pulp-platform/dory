...
void __attribute__ ((noinline))  xpulp_nn_avgpool_i2_i4(
  int8_t * pIn,
  int8_t * pOut,
  int64_t lambda,
  uint16_t out_shift,
            ...
            for (int ch_cnt = 0; ch_cnt < ch_im_in_r; ch_cnt++)
            {
              ...
              int8_t out_el = 0;
                ...
                if (flag_requant) {
                  ...
                  out_el = clips4(out_large);
                  out_large = (sum[1] * lambda + out_add) >> out_shift;
                  out_el = bitins(out_el, (int8_t) 0x0f, (int8_t) clips4(out_large), (int8_t) 0xf0, 4);
                  pDst[(ch_cnt << (1)) + 0] = out_el;
                  out_large = (sum[2] * lambda + out_add) >> out_shift;
                  out_el = bitins(out_el, (int8_t) 0xf0, (int8_t) clips4(out_large), (int8_t) 0x0f, 0);
