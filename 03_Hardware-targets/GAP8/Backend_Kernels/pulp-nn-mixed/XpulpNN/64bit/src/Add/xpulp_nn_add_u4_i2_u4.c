...
void __attribute__ ((noinline)) xpulp_nn_add_u4_i2_u4(
    uint8_t * pIn1,
    int8_t * pIn2,
    uint8_t * pOut,
    int64_t in1_mul,
    ...
    int      out_requant_flag)
{
    ...
    int64_t in1_rq1, in1_rq2, in1_rq3, in1_rq4,
             in2_rq1, in2_rq2, in2_rq3, in2_rq4;
    int64_t sum1, sum2, sum3, sum4;
    int64_t sum_out1, sum_out2, sum_out3, sum_out4;
    ...
    for (int i=0; i<(((stop-start) * ch_im_out_r * dim_im_in_x) >> 1); i++)
    {
        *((v4u*)target1_ext) = pulp_nn_u4_to_u8_r(target1);
        target1+=2;

        *((v4s*)target2_ext) = pulp_nn_i2_to_i8_r(target2);
        ...
        sum2 = clip8(in1_rq2) + clips8(in2_rq2);
#ifdef ADD_VERBOSE
        ...
        sum3 = clip8(in1_rq3) + clips8(in2_rq3);
#ifdef ADD_VERBOSE
        ...
        sum4 = clip8(in1_rq4) + clips8(in2_rq4);
#ifdef ADD_VERBOSE
        ...
        *pOutBuffer = (uint8_t) bitins(out1, (int8_t) 0x0f, out2, (int8_t) 0xf0, 4);
        pOutBuffer++;
        *pOutBuffer = (uint8_t) bitins(out3, (int8_t) 0x0f, out4, (int8_t) 0xf0, 4);
        pOutBuffer++;
    }
