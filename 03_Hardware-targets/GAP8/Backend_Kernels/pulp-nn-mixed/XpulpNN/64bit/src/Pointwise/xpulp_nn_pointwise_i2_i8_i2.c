...
void xpulp_nn_pointwise_i2_i8_i2(
                        int8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    ...
    for(int n = 0; n<((dim_out_x_r + (section * flag_dim_out_x_odd)) >> 1); n++)
    {
      ...
      pOutBuffer = xpulp_nn_matmul_i2_i8_i2(
          pIm2Col,
          pBias,
          pOutBuffer,
          pOutBuffer + ch_out_r,
    ...
    if(((dim_out_x_r + (section * flag_dim_out_x_odd)) & 0x0001))
    {
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        for(int j=0; j < (col_cnt_im2col >> 4); j++)
        {
          ...
          sum = SumDotps4(inB[2], inA[2], sum);

          sum = SumDotps4(inB[3], inA[3], sum);
        }
