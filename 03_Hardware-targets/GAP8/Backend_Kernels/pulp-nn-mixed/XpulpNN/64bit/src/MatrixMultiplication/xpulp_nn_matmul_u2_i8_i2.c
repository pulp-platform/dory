...
uint8_t * __attribute__((noinline)) xpulp_nn_matmul_u2_i8_i2(
                        uint8_t *pIn,
                        int8_t *pBias,
                        int8_t *pOut,
                        int8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 4); j++)
    {
      ...
      sum6 = MacLoad16(1, 0, 1, 1, ptrA2, sum6);
      ptrA2 = MacLoadUpdate(ptrA2);

      sum7 = MacLoad16(1, 0, 2, 1, ptrA3, sum7);
      ptrA3 = MacLoadUpdate(ptrA3);

      sum8 = MacLoad16(1, 0, 3, 1, ptrA4, sum8);
