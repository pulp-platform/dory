...
% if FLAG_BATCHNORM == 1:
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+${l2_off_k} + ${l2_off_lambda-l2_off_k};
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_lambda.ext = (uint32_t) l2_W+${l2_off_k} + ${lambda_tile_size_byte_transfer}*_i_nof_load  + ${l2_off_lambda-l2_off_k};
