...
class NEMO_onnx(ONNX_management):
    ...
    def __init__(self, onnx):
        ...
        rules = {}
        rules['Relu'] = 'To define'#'Mul-Div-Clip'
        rules['BNRelu'] = 'Mul-Add-Mul-Div-Clip'
...
############# USE A SINGLE RULE AND RENAME THE CONSTANT ARGUMENTS
        ...
        if rule in [self.rules['Relu'], self.rules['BNRelu_1'], self.rules['BNRelu_2'], self.rules['BNRelu_3']]:
            ...
            for node_iterating in (self.graph.graph.node):
                if out == node_iterating.output[0] and node_iterating.op_type == nodes_to_search[i] and i < blocks_to_search:
                    ...
                    pulp_node = self.create_node(pulp.node_element(), 0, node_iterating, self.graph)
                    pulp_node.add_parameter('input_index', inp[0])
            ...
            if input_i == out:
                ...
                if node_i.op_type in 'Add':
                    try:
                        ...
                        continue
                    for node_j in (self.graph.graph.node):
