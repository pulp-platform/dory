...
def print_template_Makefile(file_list_w, platform):
...
def print_template_network(
    ...
    for nodes in PULP_Nodes_Graph:
        if 'Gemm' in nodes.name or 'Conv' in nodes.name:
            weights_number += 1
    tk['BitIn'] = BitIn
    tk['BitW'] = BitW
    tk['BitOut'] = BitOut
...
def print_template_layer_L3(X, W, Y, fs1, fs2, padding, stride,
    ...
    if 'partial' in test_location:
        ...
        with open(save_string, "w") as f: f.write(s)
        print_template_Makefile([name[0][5:] + '_weights.hex'], 'GAP8')
...
def print_template_layer(x, y_gold, W,
    ...
    tk['y_tile_size_h'] = tile_h_out
    tk['y_tile_size_w'] = tile_w_out
    tk['y_tile_size_byte'] = int(math.ceil(tile_n_out * tile_h_out * tile_w_out * ds_y / 8.0))
    ...
    tk['tile_dim_h'] = max(int(math.ceil(float(h_out) / float(tile_h_out))), 1)
    tk['tile_dim_w'] = max(int(math.ceil(float(w_out) / float(tile_w_out))), 1)
    ...
    if n_in == tile_n_in and w_in == tile_w_in and h_in == tile_h_in and n_out == tile_n_out:
        y_buffer_size = int(math.ceil(ds_y * tile_n_out * tile_h_out * tile_w_out / 8.0))
        if DW == 0:
            W_buffer_size = int(math.ceil(ds_W * tile_n_out * tile_n_in * fs1 * fs2 / 8.0))
        else:
            W_buffer_size = int(math.ceil(ds_W * tile_n_out * 1 * fs1 * fs2 / 8.0))
    else:
        y_buffer_size = 2 * int(math.ceil(ds_y * tile_n_out * tile_h_out * tile_w_out / 8.0))
        if DW == 0:
            W_buffer_size = 2 * int(math.ceil(ds_W * tile_n_out * tile_n_in * fs1 * fs2 / 8.0))
        else:
            W_buffer_size = 2 * int(math.ceil(ds_W * tile_n_out * 1 * fs1 * fs2 / 8.0))
    if tk['FLAG_BATCHNORM'] == 1:
