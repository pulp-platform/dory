...
class Tiling():
    ...
    def __init__(self, module, out_ch, filter_size, stride, padding, groups, x_shape, L1_buffer, L2_buffer, platform, chip, test_location, BitIn, BitW, BitOut, BitActivation, optional_type):
        self.module = module
        self.out_ch = out_ch
    ...
    def get_tiling_conv2d_like(self,
        ...
        if DW == 0:
            im2col_dim = self.BitIn * 2 * 8 * fs1 * fs2 * n_in
        else:
        ...
        for iteration in range(0, 4):
            ...
            if DW == 0:
                ...
                constr_im2col = ds_x_scale * 2 * 8 * fs1 * fs2 * tile_n_in
            else:
                constr_weight = db * ds_W_scale * tile_n_in * fs1 * fs2
                constr_im2col = 32 * 8 * (8 * fs1 * (h_in) + 3) * int( 8 / min(self.BitIn, self.BitOut, self.BitW))
    ...
    def get_tiling_conv2d(self, X, Y, W,
        ...
        name_include = []
        # L3 tiling
        tiling = self.get_tiling_conv2d_L3(DW, BN, input_L3, input_dim_constraint, output_weights_dim_constraint, weight_constraint)
        if DW == 1:
            g = self.groups
            n_in = 1
        else:
        ...
        if L3_tiling == 1:
            ...
            if int(W_no_str) > 1 and (factor_h_in > 1 or factor_h_out > 1):
                print("Tiling of weights and Input/output activation from L3 not yet working. Exiting...")
                os._exit(0)
        else:
            if g > 1:
                logging.debug("  Conv2d Depthwise tiling:")
        ...
        if tiling is not None:
            ...
            L1_tiles_size = ds_x * tile_n_in * tile_h_in * tile_w_in / 8. * (1 + int(int(x_no_str) > 1)) + ds_y * tile_n_out * tile_h_out * tile_w_out / 8. * (1 + int(int(y_no_str) > 1)) + n_out * 8
            if g > 1:
    ...
    def get_tiling_pool2d(self, X, Y, W,
        ...
        if memory >= self.L2_buffer_size * 8:
            print("  Pool2d ERROR: no tiling from L3 supported. Exiting...")
            os._exit(0)            
        ...
        if collector.SolutionCount() > 0:
            ...
            in_dim1, out_dim1, weight_dim1,  l2_dim_k, l2_dim_lambda, bias_dim1, l1_dim1, n_out1, w_out1, h_out1 = print_template_layer(
                ...
                fs1, fs2, p_top, p_bottom, p_left, p_right, s,
                relu, 0, 0, out_mul, 0, out_shift, 1, 1, 1,
                name_layer=name,
                test=False,
                test_location=self.test_location,
                has_bias=True,
    ...
    def get_tiling_Add(self, X, Y, W,
        ...
        if min_tile_h_out is None:
            min_tile_h_out = 1
        # this is to renormalize all costs
        max_obj_value = self.buffer_size * cost_dim * 2 * 32
        memory = ds_x * n_in * h_in * w_in * 2 + ds_y * n_out * h_out * w_out
        if memory >= self.L2_buffer_size * 8:
            print("  Pool2d ERROR: no tiling from L3 supported. Exiting...")
            os._exit(0)            
        if memory <= self.buffer_size:
            db_x = 1
            db_y = 1
            db_W = 1
        else:
        ...
        # CONSTRAINTS: managing of correct dimensions (no decimal h_out and any
        # type of rounding)
        solver.Add(db_x * ds_x_scale * tile_n * tile_h_in * tile_w_in * 2 + db_y * ds_y_scale * tile_n * tile_h_out * tile_w_out <= 32 * self.buffer_size)
        ...
        if collector.SolutionCount() > 0:
            ...
            x_tot_str = '[%dx%dx%d]' % (n_in, h_in, w_in)
            y_tot_str = '[%dx%dx%d]' % (n_out, h_out, w_out)
            x_tot_size_str = "%.2f KiB" % (1. / 1024. * (ds_x * n_in * h_in * w_in)) if ds_x * \
                n_in * h_in * w_in > 1024 else '%d B' % (ds_x * n_in * h_in * w_in)
            y_tot_size_str = '%.2f KiB' % (1. / 1024. * (ds_y * n_out * h_out * w_out)) if ds_y * \
                n_out * h_out * w_out > 1024 else '%d B' % (ds_y * n_out * h_out * w_out)

            x_tile_str = '[%dx%dx%d]' % (tile_n, tile_h_in, tile_w_in)
            y_tile_str = '[%dx%dx%d]' % (tile_n, tile_h_out, tile_w_out)

            x_size_str = "%.2f KiB" % (1. / 1024. * (ds_x * tile_n * tile_h_in * tile_w_in)) if ds_x * \
                tile_n * tile_h_in * tile_w_in > 1024 else '%d B' % (ds_x * tile_n * tile_h_in * tile_w_in)
            y_size_str = "%.2f KiB" % (1. / 1024. * (ds_y * tile_n * tile_h_out * tile_w_out)) if ds_y * \
                tile_n * tile_h_out * tile_w_out > 1024 else '%d B' % (ds_y * tile_n * tile_h_out * tile_w_out)
