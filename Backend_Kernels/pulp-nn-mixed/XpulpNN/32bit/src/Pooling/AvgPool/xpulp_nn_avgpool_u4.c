...
void __attribute__ ((noinline)) xpulp_nn_avgpool_u4(
  uint8_t * pIn,
  uint8_t * pOut,
  ...
  uint16_t  stride_y)
{
  ...
  for (i_y = start; i_y < stop; i_y++)
  {
    for (i_x = 0; i_x < dim_im_out_x; i_x++)
    {
      ...
      {
        xpulp_nn_avg_and_replace_u4(target, win_start, ch_im_in_r);
      }
    }
  }
  ...
  for (i_y = start2; i_y < stop2; i_y++)
  {
    ...
    {
      xpulp_nn_avg_and_replace_u4(target, row_start, dim_im_out_x * ch_im_in_r);
    }
  }
