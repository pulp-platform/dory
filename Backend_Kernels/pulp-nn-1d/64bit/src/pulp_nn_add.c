...
)
{
	...
	int chunk = (dim_im_in_w >> Log2Core) + ((dim_im_in_w & (NUM_CORES-1))!=0);

	int start = min(chunk * core_id, dim_im_in_w);
	int stop = min(start + chunk, dim_im_in_w);
	uint8_t *target1 = Im_in_1 + start*ch_im_in;
	uint8_t *target2 = Im_in_2 + start*ch_im_in;
	uint8_t *pOut = Im_out + start*ch_im_in;

	for (int spatial = 0; spatial<ch_im_in*(stop-start); spatial+=1)
	{
		*pOut = pulp_nn_add_quant_u8(*target1, *target2, out_mult1, out_mult2, out_mult3, out_shift, out_shift2);
		target1 += 1;
		target2 += 1;
		pOut += 1;
	}
