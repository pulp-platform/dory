...
uint8_t __attribute__ ((noinline)) *pulp_nn_matmul_4x2_uint8_indirect(
	int8_t * pWeight,
	uint8_t ** pInBuffer,
	uint16_t ch_out,
	...
	for (int i = 0; i < ch_out>>2; i++){
		...
		for(int j = 0; j <  dim_kernel_y; j++){
			...
			uint8_t *pB2=*(pInBuffer+dim_kernel_y+j);
		...
		if (flag_batch_norm==1 && flag_relu==1){
			...
			*pOut2 = pulp_nn_bn_quant_u8(sum5, (int64_t) *k, (int64_t) *lambda, out_shift);
      			pOut++;
			pOut2++;
      			k++;
      			lambda++;
			...
			*pOut2 = pulp_nn_bn_quant_u8(sum6, (int64_t) *k, (int64_t) *lambda, out_shift);
      			pOut++;
			pOut2++;
      			k++;
      			lambda++;
			...
			*pOut2 = pulp_nn_bn_quant_u8(sum7, (int64_t) *k, (int64_t) *lambda, out_shift);
      			pOut++;
			pOut2++;
      			k++;
      			lambda++;
			...
			*pOut2 = pulp_nn_bn_quant_u8(sum8, (int64_t) *k, (int64_t) *lambda, out_shift);
      			pOut++;
			pOut2++;
      			k++;
      			lambda++;
	...
	while(chan_left){
		...
		if (flag_batch_norm==1 && flag_relu==1){
      			*pOut = pulp_nn_bn_quant_u8(sum1, (int64_t) *k, (int64_t) *lambda, out_shift);
      			pOut++;
			*pOut2= pulp_nn_bn_quant_u8(sum2, (int64_t) *k, (int64_t) *lambda, out_shift);
			pOut2++;
      			k++;
      			lambda++;
   		}
			...
			*pOut = (int8_t) clip8(sum1 >> out_shift);
        		pOut++;
