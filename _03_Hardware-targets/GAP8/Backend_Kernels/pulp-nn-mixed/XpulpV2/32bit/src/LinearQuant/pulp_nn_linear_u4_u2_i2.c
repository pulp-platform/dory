                        ...
                        uint8_t flag_batch_norm)
{
    ...
    for(i=start; i<stop; i+=4)
    {
        ...
        for (int j=0; j<(dim_vec >> 4); j++)
        {
          ...
          sum3 = SumDotp4(vecA[2], vecB3[2], sum3);
          sum3 = SumDotp4(vecA[3], vecB3[3], sum3);
        ...
        while (col_cnt)
        {
          ...
          pA++;
          uint8_t inA3 = (uint8_t) bitextu((uint32_t) *pA, 4, 0);
          uint8_t inA4 = (uint8_t) bitextu((uint32_t) *pA, 4, 4);
          pA++;
