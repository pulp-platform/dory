...
void __attribute__ ((noinline)) xpulp_nn_maxpool_i4(
                                                  int8_t * pIn,
                                                  int8_t * pOut,
  ...
  uint16_t  stride_y)
{
  ...
  for (i_y = start; i_y < stop; i_y++)
  {
    for (i_x = 0; i_x < dim_im_out_x; i_x++)
    {
      ...
      {
        xpulp_nn_compare_and_replace_if_larger_i4(target, win_start, ch_im_in_r);
      }
    }
  }
  ...
  for (i_y = start2; i_y < stop2; i_y++)
  {
    ...
    {
      xpulp_nn_compare_and_replace_if_larger_i4(target, row_start, dim_im_out_x * ch_im_in_r);
    }
  }
  pi_cl_team_barrier(0);
}
