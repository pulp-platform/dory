...
void __attribute__ ((noinline)) xpulp_nn_add_u2_i2_i4(
    uint8_t * pIn1,
    int8_t * pIn2,
    int8_t * pOut,
    int64_t in1_mul,
    ...
    int      out_requant_flag)
{
    ...
    for (int i=0; i<(((stop-start) * ch_im_out_r * dim_im_in_x) >> 1); i++)
    {
        ...
        *pOutBuffer = (int8_t) bitins(out1, (int8_t) 0x0f, out2, (int8_t) 0xf0, 4);
        pOutBuffer++;
        *pOutBuffer = (int8_t) bitins(out3, (int8_t) 0x0f, out4, (int8_t) 0xf0, 4);
        pOutBuffer++;
    }
