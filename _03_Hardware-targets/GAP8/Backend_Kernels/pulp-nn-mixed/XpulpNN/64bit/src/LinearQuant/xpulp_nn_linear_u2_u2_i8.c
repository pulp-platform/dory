...
void __attribute__((noinline)) xpulp_nn_linear_u2_u2_i8(
                        uint8_t *pIn,
                        int8_t *pBias,
                        uint8_t *pOut,
                        int8_t *pWeight,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(i=start; i<stop; i+=4)
  {
    ...
    for(int j=0; j < (dim_vec >> 4); j++)
    {
      ...
      sum3 = MacLoad4(1, 0, 2, 0, ptrA3, sum3);
      ptrA3 = MacLoadUpdate(ptrA3);
      sum4 = MacLoad4(1, 0, 3, 0, ptrA4, sum4);
