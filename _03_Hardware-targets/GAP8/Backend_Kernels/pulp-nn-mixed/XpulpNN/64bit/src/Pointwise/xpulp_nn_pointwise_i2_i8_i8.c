...
void xpulp_nn_pointwise_i2_i8_i8(
                        int8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  int8_t *pIm2Col = pIm2ColBuffer + (2 * core_id * PACK_INT8_SIZE(ch_in));
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    ...
    {
      xpulp_nn_im2col_i2_to_i8(pIn + (i_out_x * ch_in_r) + (i_out_y * dim_in_x * ch_in_r), pIm2Col, ch_in<<1);
      pOutBuffer = xpulp_nn_matmul_i2_i8_i8(
          pIm2Col,
          pBias,
          pOutBuffer,
          pOutBuffer + ch_out_r,
    ...
    if(((dim_out_x_r + (section * flag_dim_out_x_odd)) & 0x0001))
    {
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        for(int j=0; j < (col_cnt_im2col >> 2); j++)
        {
          ...
          sum = SumDotps4(inB[0], inA, sum);

          inA = *((v4s*) pA);

          pA+=4;

          sum = SumDotps4(inB[1], inA, sum);

          inA = *((v4s*) pA);

          pA+=4;

          sum = SumDotps4(inB[2], inA, sum);

          inA = *((v4s*) pA);

          pA+=4;

          sum = SumDotps4(inB[3], inA, sum);
        }
