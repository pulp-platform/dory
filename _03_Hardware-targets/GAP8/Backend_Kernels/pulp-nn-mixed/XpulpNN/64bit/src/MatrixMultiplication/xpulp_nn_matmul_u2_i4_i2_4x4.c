/*
 * xpulp_nn_matmul_u2_i4_i2_4x4.c
...
uint8_t * __attribute__((noinline)) xpulp_nn_matmul_u2_i4_i2_4x4(
                        uint8_t *pIn,
                        int8_t *pBias,
                        int8_t *pOut,
                        int8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 4); j++)
    {
      ...
      sum2 = MacLoad16(0, 0, 1, 0, ptrA2, sum2);
      sum3 = MacLoad16(0, 0, 2, 0, ptrA3, sum3);
      sum4 = MacLoad16(0, 1, 3, 0, ptrB3, sum4);
      ptrB3 = MacLoadUpdate(ptrB3);
      

      sum5 = MacLoad16(0, 0, 0, 1, ptrA, sum5);
      sum6 = MacLoad16(0, 0, 1, 1, ptrA2, sum6);
      sum7 = MacLoad16(0, 0, 2, 1, ptrA3, sum7);
      sum8 = MacLoad16(0, 1, 3, 1, ptrB4, sum8);
      ptrB4 = MacLoadUpdate(ptrB4);

      sum9  = MacLoad16(0, 0, 0, 0, ptrA, sum9);
      sum10 = MacLoad16(0, 0, 1, 0, ptrA2, sum10);
      sum11 = MacLoad16(0, 0, 2, 0, ptrA3, sum11);
      sum12 = MacLoad16(0, 1, 3, 0, ptrB, sum12);
      ptrB = MacLoadUpdate(ptrB);

      sum13 = MacLoad16(1, 0, 0, 1, ptrA, sum13);
      ptrA  = MacLoadUpdate(ptrA);
      sum14 = MacLoad16(1, 0, 1, 1, ptrA2, sum14);
      ptrA2 = MacLoadUpdate(ptrA2);
      sum15 = MacLoad16(1, 0, 2, 1, ptrA3, sum15);
      ptrA3 = MacLoadUpdate(ptrA3);
      sum16 = MacLoad16(1, 0, 3, 1, ptrA4, sum16);
  ...
  while(chan_left)
  {
    ...
    if(col_cnt_im2col)
    {
      ...
      do
      {
        ...
        int8_t inB = (uint8_t)bitextu((uint32_t) *pB, 2, 0);
        int8_t inB2 = (uint8_t)bitextu((uint32_t) *pB2, 2, 0);
