...
void __attribute__ ((noinline))  xpulp_nn_avgpool_u4_i2(
  uint8_t * pIn,
  int8_t * pOut,
  int32_t lambda,
  uint16_t out_shift,
              ...
              sum[1] = 0;
                ...
                if (flag_requant) {
                  out_large = (sum[0] * lambda + out_add) >> out_shift;
                    out_el |= ((clips2(out_large) & 0x03) << (in_iter_cnt * 4 + 0));
                  out_large = (sum[1] * lambda + out_add) >> out_shift;
                    out_el |= ((clips2(out_large) & 0x03) << (in_iter_cnt * 4 + 2));
                  } else {
