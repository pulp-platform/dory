...
uint8_t * __attribute__((noinline)) xpulp_nn_matmul_i2_u8_i2(
                        int8_t *pIn,
                        int8_t *pBias,
                        uint8_t *pOut,
                        uint8_t *pOut2,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for(int i=0; i < (ch_out >> 2); i++)
  {
    ...
    for(int j=0; j<(num_col_im2col >> 4); j++)
    {
      ...
      sum4 = MacLoads16(0, 1, 3, 0, ptrB, sum4);
      ptrB = MacLoadUpdate(ptrB);
      

      sum5 = MacLoads16(1, 0, 0, 1, ptrA, sum5);
      ptrA = MacLoadUpdate(ptrA);

      sum6 = MacLoads16(1, 0, 1, 1, ptrA2, sum6);
      ptrA2 = MacLoadUpdate(ptrA2);

      sum7 = MacLoads16(1, 0, 2, 1, ptrA3, sum7);
      ptrA3 = MacLoadUpdate(ptrA3);

      sum8 = MacLoads16(1, 0, 3, 1, ptrA4, sum8);
  ...
  while(chan_left)
  {
    ...
    for(int j=0; j < (num_col_im2col >> 4); j++)
    {
      ...
      sum  = MacLoads16(0, 1, 0, 0, ptrB, sum);
      ptrB = MacLoadUpdate(ptrB);

      sum2 = MacLoads16(1, 0, 0, 1, ptrA, sum2);
      ptrA = MacLoadUpdate(ptrA);
    }
    int col_cnt_im2col = num_col_im2col & 0xf;
    ...
    if(col_cnt_im2col)
    {
      ...
      do
      {
        ...
        uint8_t inB = (int8_t)bitext((int32_t) *pB, 2, 0);
        uint8_t inB2 = (int8_t)bitext((int32_t) *pB2, 2, 0);
