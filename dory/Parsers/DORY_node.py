...
class DORY_node:
    ...
    def export_to_onnx(self):
        ...
        for key, value in self.__dict__.items():
            if not isinstance(value, np.ndarray) and not isinstance(value,str) and not isinstance(value,dict) and not isinstance(value,__class__) and not isinstance(value,type(None)) and key not in added_parameters and not isinstance(value, bool):
