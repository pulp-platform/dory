...
class Tiler_Fused_PULP():
    ...
    def get_tiling_conv2d_L3(self):
        ...
        for iteration in range(0, 4):
            ...
            if g1 == 1:
                im2col_dimension_L1 = CORES * (ks2[0] * (intermediate_ch + p2[0] + p2[2]) + ks2[0])
            else:
                im2col_dimension_L1 = CORES * (ks1[0] * (in_ch + p1[0] + p1[2]) + ks1[0])
            constants_0 = 0
            for name in self.HW_node.node0.constant_names:
                if name in ["l","k"]:
                    constants_0+=1
            ...
            constraint_all_L1 = input_tile_dimension_L1 + output_tile_dimension_L1 + weight_tile_dimension_L1 + constants_tile_dimension_L1 + im2col_dimension_L1 + 20 
            ...
            solver.Add(obj_expr == (constraint_all 
                                    + 10000 * (constraint_all_L1 < L1_memory)
                                    + 100000 * (((tile_h_intermediate) % IB_FACTOR) == 0)
                                    + 100000 * ((tile_h_out % IB_FACTOR) == 0)
                                    + 1000 * ((tile_n_out) % 4 == 0)))
    ...
    def get_tiling_conv2d_L2(self): 
        ...
        buffer_total = self.HW_node.tiling_dimensions["L2"]["weight_memory"] + self.HW_node.tiling_dimensions["L2"]["constants_memory"] + self.HW_node.tiling_dimensions["L2"]["bias_memory"] + in_mem + out_mem + im2col_dim
        ...
        if g0 == 1:
            solver.Add(tile_n_out == tile_n_intermediate)
        if g1 == 1:
        ...
        if g0 == 1:
            weight_tile_dimension = db * tile_n_in * tile_n_intermediate * np.prod(ks0)
        else:
            weight_tile_dimension = db * tile_n_in * np.prod(ks0)
        if g1 == 1:
            weight_tile_dimension += db * tile_n_intermediate * tile_n_out * np.prod(ks1)
        else:
            weight_tile_dimension += db * tile_n_intermediate * np.prod(ks1)
        ...
        constraint_all = self.HW_node.tiling_dimensions["L2"]["bias_memory"] + input_tile_dimension + output_tile_dimension + weight_tile_dimension + constants_tile_dimension + im2col_dimension + 40 
        ...
        if g0 == 1:
            ...
            heuristics += 100000000000 * ((tile_n_out % IB_FACTOR) == 0)
            heuristics += 100000 * ((tile_h_in % 4) == 0)
            heuristics += 100000 * ((tile_w_in % 2) == 0)
            heuristics += 10000 * ((tile_w_out % 2) == 0)
            heuristics += 10000 * ((tile_h_out % 2) == 0)
            heuristics += 100000 * (tile_h_in * tile_w_in >= 16)
            heuristics += 1000 * tile_h_out
            heuristics += 100 * tile_w_out
            heuristics += 1 * tile_n_out 
        if g1 == 1:
            heuristics += constraint_all
            heuristics += 10000000 * ((tile_h_out % IB_FACTOR) == 0)
            heuristics += 10000 * ((tile_w_out % 2) == 0)
            heuristics += 10000 * ((tile_n_out - 1) % 4) 
            heuristics += 100000 * (tile_w_out * tile_h_out >= 16)
            heuristics += 1000000 * (tile_n_out == out_ch)
            heuristics += 100 * tile_n_out
            heuristics += 1 * tile_w_out 
        ...
        if collector.SolutionCount() > 0:
            ...
            input_tile_dimension = tile_n_in * tile_h_in * tile_w_in
            output_tile_dimension = tile_n_out * tile_h_out * tile_w_out
            if g0 == 1:
                weight_tile_dimension = tile_n_out * 9 + tile_n_in * tile_n_out
            if g1 == 1:
                weight_tile_dimension = tile_n_in * 9 + tile_n_in * tile_n_out
            constants_tile_dimension = tile_n_in * 8 + tile_n_out * 8
            im2col_dimension = (((((tile_n_in + 2) * 3) + 3) * 8) + ((tile_n_in * tile_w_in) * 4))
            constraint_all = self.HW_node.tiling_dimensions["L2"]["bias_memory"] + input_tile_dimension + output_tile_dimension + weight_tile_dimension + constants_tile_dimension + im2col_dimension + 40 
