...
class Model_deployment_Occamy(Model_deployment):
    ...
    def copy_files(self, optional, layer_mixed_list,version, sdk, backend, dma_parallelization):
        ...
        tmpl = Template(filename=root + f"/templates_{backend}/dory.h")
        s = tmpl.render(**tk)
        save_string = './application/DORY_network/inc/dory.h'
        with open(save_string, "w") as f:
            f.write(s)
        os.system(f'cp ../templates_{backend}/mem_controller.c  ./application/DORY_network/src/')
        os.system(f'cp ../templates_{backend}/mem_controller.h  ./application/DORY_network/inc/')
        tk = OrderedDict([])
        tk['chip'] = self.chip
        tk['dma_parallelization'] = dma_parallelization
        tmpl = Template(filename=root+f"/templates_{backend}/dory.c")
        s = tmpl.render(**tk)
        save_string = './application/DORY_network/src/dory.c'
        with open(save_string, "w") as f:
            f.write(s)
        os.system(f'cp ../templates_{backend}/test_template.c ./application/DORY_network/src/')
        os.system(f'cp ../templates_{backend}/network.h ./application/DORY_network/inc/')
        os.system('cp ../occamy_convolutions/include/*  ./application/DORY_network/inc/')
        os.system('cp ../occamy_convolutions/src/* ./application/DORY_network/src/')
