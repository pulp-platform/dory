...
// aggiungere padding su tutti i lati, acc_out, and filter asymettric
  % if 'Max' in optional:
    % if precision_library == 'mixed-sw':
    pulp_nn_maxpool_${data_type_y[0]}${y_data_size_byte}(
    % elif precision_library == 'mixed-hw':
    xpulp_nn_maxpool_${data_type_y[0]}${y_data_size_byte}(
    % else:
    pulp_nn_maxpool(
    % endif
  % else:
    % if 'mixed' in precision_library:
    ${"x" if "hw" in precision_library else ""}pulp_nn_avgpool_${data_type_x[0]}${x_data_size_byte}_${data_type_y[0]}${y_data_size_byte}(
    % else:
    pulp_nn_avgpool(
    % endif
  % endif
