...
class Tiler_Add():
    ...
    def get_tiling_Add_L2(self):
        ...
        ###############################################
        buffer_total = self.node.tiling_dimensions["L2"]["constants_memory"] + self.node.tiling_dimensions["L2"]["input_activation_memory"] * 2 + self.node.tiling_dimensions["L2"]["output_activation_memory"]
        ...
        # type of rounding)
        input_tile_dimension  = (db * in_ch * tile_h_in * inp_dim[1] * self.node.input_activation_bits + 7) // 8 # the 7 is to account for bit precision of 1, which still occupy an entire byte
        output_tile_dimension = (db * out_ch * tile_h_out * out_dim[1] * self.node.output_activation_bits + 7) // 8 # the 7 is to account for bit precision of 1, which still occupy an entire byte
