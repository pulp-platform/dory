...
static void execute_prepare(Layer tile, nnx_task_t * const task) {
    ...
    task->data.cfg.padding = (tile.padding.top << 28)
        | (tile.padding.right << 24)
        | (tile.padding.bottom << 20)
        | (tile.padding.left << 16)
        | 0;

    task->data.infeat_ptr = padded_input_ptr(tile.addr.input, tile.padding.top,
                                        tile.padding.left, tile.input.width,
                                        tile.input.channel, 8);
    task->data.weights_ptr = tile.addr.weights;
    task->data.scale_ptr = tile.addr.scale;
    task->data.scale_bias_ptr = tile.addr.bias;
    task->data.scale_shift_ptr = 0;
...
static int execute_stride2x2_blocking(nnx_task_t task, Layer tile, Kernel kernel) {
    ...
    for (int i = 0; i < n_h; i++) {
        for (int j = 0; j < n_w; j++) {
            task.data.infeat_ptr = dory_get_tile_3d(input_base,
                                               i, j, 0,                    /* index */
            ...
            task.data.cfg.padding = get_padding(i, j, n_h, n_w, tile_padding);

            last_job_id = nnx_acquire_blocking();
            if (i_job < 2) {
                nnx_offload(&task);
            } else {
                NE16_WRITE_IO_REG(NE16_REG_INFEAT_PTR, task.data.infeat_ptr);
                NE16_WRITE_IO_REG(NE16_REG_OUTFEAT_PTR, task.data.outfeat_ptr);
                NE16_WRITE_IO_REG(NE16_REG_PADDING, task.data.cfg.padding);
            }
            nnx_run_async();
